...
//         } 
    ...
    stages {
        ...
        stage('clean package') {
         ...
         stage ('NEXUS DEPLOY') {
            steps {
                sh 'mvn clean package deploy:deploy-file -DgroupId=com.esprit.examen -DartifactId=tpAchatProject -Dversion=1.0 -DgeneratePom=true -Dpackaging=jar -DrepositoryId=deploymentRepo -Durl=http://172.10.0.140:8081/repository/maven-snapshots/ -Dfile=target/tpAchatProject-1.0.jar -DskipTests'
            }
        }
        stage('mvn test') {
            steps {
